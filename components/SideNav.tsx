...
// const navbar = () => {
//   return (
//     <div className="bg-slate-950 text-slate-100 flex h-screen">
//       <SideNav />
//       <div>
//         <Header />
//         <Home />
...
const SideNav = () => {
  ...
  return (
    ...
    <nav className="bg-stone-950 flex flex-col items-center h-screen sticky top-0 left-0">
...
const NavItem = ({ children, selected, id, setSelected } : any) => {
  return (
    <motion.button
      className="p-4 text-lg hover:bg-gray-950 hover:border-r hover:border-solid hover:border-indigo-500  transition-colors relative"
      onClick={() => setSelected(id)}
    >
      <span className={`block relative z-10 vertical-rl  ${selected ? 'text-gray-300' : 'text-gray-600'} hover:text-gray-400`}>{children}</span>
      <AnimatePresence>
        {selected && (
          <motion.span
            className="absolute inset-0 border-r border-solid border-indigo-500 z-0 bg-gray-950"
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            exit={{ scale: 0 }}
          >
